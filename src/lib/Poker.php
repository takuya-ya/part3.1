...
function showDown (string $card1A, string $card1B, string $card2A, string $card2B): array
{
    ...
    // その際、judgeWinnerでのcompareCardNumによる役が同じで手札のランクで勝負,A-2のパターン処理もここで行い、勝負に必要な要素をプレイヤー毎に配列化してreturn。私は、役名、役のランク、カードの強弱をそれぞれ配列を作成している状態。$handRanks[1, 0]のように。プレイヤー毎に情報をまとめればいい。
    // カードの役を取得
...
function judgeWinner(array $roleRanks, array $handRanks, array $handRoles): int
{
...
function compareCardNum(array $handRanks, array $handRoles): int
{
